...
nconf.defaults({
  ...
  'google_spreadsheet_key' : '1so945rTRPwX0KSDTqo-I-CwNoOoY5v0JOMLv04KW29M',
...
try {
  var secrets = JSON.parse(fs.readFileSync(secretFile));
} catch (e) {
  if (e.code === 'ENOENT') {
...
function primeToken(client, cb) {
  fs.readFile(untildify(nconf.get('MH_APP_TOKEN')), 'utf8', function(err, data) {
